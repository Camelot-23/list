...
struct QNRTCSetting {
  ...
  QNAudioScene audio_scene = kDefault;
};
...
struct QNMicrophoneAudioTrackConfig {
  QNAudioQuality audio_quality;
  bool communication_mode_on; // 是否开启通话模式
  string tag;  // 自定义标签
};
...
class QINIU_EXPORT_DLL QNRTC {
 public:
   ...
   * 设置 QNRTCClient 事件监听
   *
   * listener 对象生命周期须在 DestroyClient 前保证有效;
   * CreateClient() 后立即设置，不支持多次设置
   */
   ...
   * 开启音频处理文件 Dump。
   */
  static bool StartAecDump(const std::string& file_path, int64_t max_size_bytes);

  /**
   * 停止音频处理文件 Dump。
   */
  static void StopAecDump();
